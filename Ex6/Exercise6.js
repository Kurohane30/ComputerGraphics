...
function defineObjects() {
    drawingObjects.wiredCube = new WireFrameCube(gl, [1.0, 1.0, 0.0, 0.1]);
    drawingObjects.solidCube = new SolidCube(gl,
        [1.0, 0.0, 0.0, 0.1], // back color
        [0.0, 1.0, 0.0, 0.1], // front color
        [0.0, 0.0, 1.0, 0.1], // right color
        [1.0, 1.0, 0.0, 0.1], // left color
        [0.0, 1.0, 1.0, 0.1], // top color
        [1.0, 0.0, 1.0, 0.1]); // bottom color
...
function draw() {
    ...
    mat3.normalFromMat4(normalMatrix, modelViewMatrix);
    gl.uniformMatrix3fv(ctx.uNormalMatrixId, false, normalMatrix);
    drawingObjects.solidSphere.drawWithColor(gl, ctx.aVertexPositionId, ctx.aVertexColorId, ctx.aVertexNormalId, [0, 0, 1]);
}
